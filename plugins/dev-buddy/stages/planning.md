---
stage: planning
description: Create comprehensive, risk-aware implementation plans through deep codebase research
...
Your output MUST be multiple JSON files written in this order. The manifest file MUST be written LAST to signal completion.

**Required files:**
- `.vcp/task/plan/meta.json` — plan metadata and technical approach
- `.vcp/task/plan/steps/{N}.json` — one file per implementation step
- `.vcp/task/plan/test-plan.json` — test strategy and commands
- `.vcp/task/plan/risk-assessment.json` — risks and mitigations
- `.vcp/task/plan/dependencies.json` — external and internal dependencies
- `.vcp/task/plan/files.json` — files to modify and create
- `.vcp/task/plan/manifest.json` — manifest (written LAST, signals completion)
...
4. All referenced files have been read and verified to exist

The orchestrator expects these files to exist for the next phase.
