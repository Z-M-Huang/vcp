...
/dev-buddy-once use local-cli o3 model to refactor the auth module
...
- **Provider name** — the preset name (e.g., "minimax", "local-cli", "anthropic-subscription")
...
3. **Multiple matches** → list the matches and ask the user to pick
...
**IMPORTANT:** The Bash tool has a hard max timeout of 600,000ms (10 min). CLI tasks can run much longer than that. Always use `run_in_background: true` to prevent the Bash tool from killing the process prematurely.
...
The CLI tool runs directly in the project directory using the configured command. Its output streams to the terminal.
...
| Multiple preset matches | Ask the user to choose from the matching names |
