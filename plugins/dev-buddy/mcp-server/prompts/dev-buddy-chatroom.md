...
Fan out a topic to ALL configured AIs + the local moderator simultaneously, synthesize the best approach, iterate until consensus.
...
```bash
bun -e "
import { loadChatroomConfig } from '<pluginRoot>/scripts/chatroom-config.ts';
import { readPresets } from '<pluginRoot>/scripts/preset-utils.ts';
import { validateChatroomConfig } from '<pluginRoot>/scripts/chatroom-config.ts';
...
2. Compute project hash:
   ```bash
   bun -e "const c=require('crypto');console.log(c.createHash('sha256').update(process.env.PROJECT_ROOT||process.cwd()).digest('hex').slice(0,8))"
   ```
...
**PARALLEL OK — dispatch all participants + generate local moderator's position in a single message.**
...
```bash
bun -e "
import { getSystemPrompt } from '<pluginRoot>/scripts/system-prompts.ts';
const prompt = getSystemPrompt('{SYSTEM_PROMPT}', '<pluginRoot>/system-prompts/built-in');
...
- **API:** `Bash(run_in_background: true)` →
  ```bash
  bun "<pluginRoot>/scripts/one-shot-runner.ts" \
    --type api --output-id cr-{SESSION_ID}-p{i}-r1 \
    --preset "{PRESET}" --model "{MODEL}" \
    --cwd "<projectRoot>" --task-stdin <<'{DELIM}'
  {prompt_text}
  {DELIM}
  ```
...
### 2c. the local moderator generates its own opening position

While background tasks run, generate your own analysis of the topic inline. This is local moderator's opening position in the debate.
...
- **Quorum rule:** Need at least 1 external response + local moderator's to proceed. If ALL external participants fail, report the error to the user and stop.

---

## Step 4: Recap, Synthesize & Check Consensus

Read ALL collected responses (local moderator's + all external participants).
...
This ledger feeds Steps 4b, 5, and 6. It is local moderator's internal state — not sent to participants.
...
**The local moderator MUST generate an updated synthesis every round.** This synthesis:
...
**local moderator's round N response:** While background tasks run, produce a local response following the same 4-section structure (CRITIQUES, DEFENSE, UPDATED POSITION, VERDICT) inline.
...
1. **Participant repo mutation:** Both API participants (via api-task-runner's Write/Edit/Bash tools) and CLI participants (via their native shell access, e.g., a configured CLI command) retain the ability to modify the repo despite the prompt instruction to only read and analyze. This is prompt-level enforcement only. A structural read-only mode is a follow-up feature.
...
- Do NOT skip local moderator's position — the local moderator is always a participant
