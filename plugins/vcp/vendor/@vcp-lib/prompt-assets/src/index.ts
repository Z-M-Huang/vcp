 ...
 * 1. Built-in: directory passed by the caller (e.g. plugin's system-prompts/built-in/)
 * 2. Custom: ~/.vcp/system-prompts/*.md (user-created)
 *
 * Loads stage definitions from a stages directory passed by the caller
 * (e.g. plugin's stages/*.md, one per StageType).
 *
...
import type { SystemPrompt, StagePrompt } from './types.ts';

export type { SystemPrompt, StagePrompt };
...
const CUSTOM_PROMPTS_DIR = path.join(os.homedir(), '.vcp', 'system-prompts');
 ...
 *
 * @param builtInDir - Path to the agents directory (e.g. plugin's system-prompts/built-in/)
...
/**
 * Write a custom system prompt to ~/.vcp/system-prompts/.
 ...
 * @param stagesDir - Path to the stages directory (caller-provided)
