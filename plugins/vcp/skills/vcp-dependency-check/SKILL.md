...
This skill is a thin slash-command launcher. The authoritative workflow lives in the VCP MCP prompt `vcp_dependency_check` and resource `vcp://prompts/vcp-dependency-check`.

1. Fetch the MCP prompt `vcp_dependency_check` with the caller-supplied host, current project path, and raw command arguments.
2. If MCP prompts are not directly available, call VCP MCP tool `get_prompt({ command: "vcp-dependency-check", host, project_path, arguments })` and follow the returned prompt text.
