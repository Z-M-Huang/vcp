...
4. Use the structured result. It contains: `applicableStandards`, `ignoredRules`, `severity`, `exclude`, and project config values.
5. Use the project config `frameworks` field to determine which package ecosystem(s) to check. If no `frameworks` are listed, auto-detect by looking for manifest files (package.json, requirements.txt, pyproject.toml, pom.xml, build.gradle, Gemfile, go.mod, Cargo.toml).
