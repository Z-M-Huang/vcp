...
This prompt is host-neutral. Where it needs user confirmation:

- Use the caller's native interactive confirmation mechanism when one is available.
- If no interactive confirmation tool is available, print the question and the exact re-invocation command, then stop. Examples: `--confirm` for risky security ignores, `--force` to bypass an unknown-standard warning. The user re-invokes; the workflow detects the flag and skips the prompt.
...
If the intent is ambiguous: ask one clarifying question if the caller supports interactive input. Otherwise, tell the user "Please rephrase your request with an explicit action — for example: `/vcp-config add-ignore core-architecture/rule-3` or `/vcp-config enable-scope database`." and stop. Do not guess.
...
- For standard IDs: check that the `id` exists in any scope manifest's `standards` array. If not found, warn the user: "Standard '[id]' not found in the manifest. Available standards: [list ids]." If interactive confirmation is available, ask whether to add it anyway. Otherwise, instruct the user: "Re-run with `--force` to add it anyway." and stop.
...
If the action adds an ignore for any of these, **always** require explicit confirmation. If interactive confirmation is available, ask for confirmation. Otherwise, print the warning and instruct the user: "Re-run the command with `--confirm` to suppress this finding." Then stop. The targets requiring confirmation are:
...
For global config `reset`, always require explicit confirmation. If interactive confirmation is available, ask: "This will delete `~/.vcp/config.json`. All projects will need to re-run `/vcp-init`. Are you sure?" Otherwise, print the warning and instruct the user to re-run with `/vcp-config global reset --confirm`.
...
| **Plugin root** | <plugin-root-from-config> | global |
| **Debug** | false | global |
...
| **Plugin root** | <plugin-root-from-config> |
